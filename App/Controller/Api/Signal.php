...
class Signal extends ApiController {
  ...
  public function __construct() {
    ...
    $this->methodIn(function() {
      $this->device = \M\Device::one('id = ?', Router::param('deviceId'));
      $this->event = \M\Event::one('id = ?', Router::param('eventId'));
      return $this->device && $this->event;
    });
  }
  ...
  public function create() {
    ...
    $params = Input::ValidPost(function($params) {
      ...
      Validator::optional($params, 'signals', '訊號')->default([])->isArray(0)->map(function($signal) use ($status) {
        ...
        $signal['deviceId'] = $this->device->id;
      ...
      $last = \M\Signal::last(['order' => 'timeAt, id DESC', 'where' => ['deviceId = ? AND eventId = ? AND enable = ?', $this->device->id, $this->event->id, \M\Signal::ENABLE_YES]]);
  ...
  //   $params = Input::ValidPost(function($params) use (&$this->event) {
  //     Validator::must($params, 'deviceId', '裝置 ID')->isId();
  //     $this->device = \M\Device::one('id = ?', $params['deviceId']);
  //     $this->device || error('找不到正確的裝置！');

  //     Validator::must($params, 'eventId', '活動 ID')->isId();
  //     $this->event = \M\Event::one('id = ?', $params['eventId']);
  //     $this->event || error('找不到正確的活動！');
  ...
  //     if (!$last = \M\Signal::last('deviceId = ? AND eventId = ? AND enable = ?', $this->device->id, $this->event->id, \M\Signal::ENABLE_YES))
  //       return $params;
  ...
  //     $params['enable'] == \M\Signal::ENABLE_NO
  //       && count(array_filter(\M\Signal::all(['select' => 'enable', 'order' => 'id desc', 'limit' => \M\Signal::RE_COUNT, 'where' => ['deviceId = ? AND eventId = ?', $this->device->id, $this->event->id]]), function($signal) { return $signal->enable == \M\Signal::ENABLE_NO; })) >= \M\Signal::RE_COUNT
  //       && ($params['enable'] = \M\Signal::ENABLE_YES)
  ...
  //   $length = 0;
  //   for ($i = 1, $s = $this->event->getSignals(), $c = count($s); $i < $c; $i++)
  //     $length += Tool::length($s[$i - 1]['lat'], $s[$i - 1]['lng'], $s[$i]['lat'], $s[$i]['lng']);

  //   $this->event->length = round($length, 2);

  //   transaction(function() use (&$params, &$obj, &$this->event) {
  //     $obj = \M\Signal::create($params);
  //     return $obj && $this->event->save();
  //   });
