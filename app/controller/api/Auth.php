...
class Auth extends ApiController {
   ...
   * @apiSuccess {Object} user         登入的使用者
   * @apiSuccess {Object} user.id      使用者的 ID
   * @apiSuccess {Object} user.name    使用者的名稱
   * @apiSuccess {Object} user.avatar  使用者的頭像
   * @apiSuccess {String} type         使用者登入類型[normal, facebook, google]
   * @apiSuccessExample {json} 成功:
   *     HTTP/1.1 200 OK
   *     {
   *         "token": "138e286e95a9e47ce59d4fdb3e744cf0",
   *         "user": {
   *             "id": 35,
   *             "name": "OA",
   *             "avatar": ""
  ...
  public function login () {
    ...
    $validation = function(&$posts, &$user) {
      ...
      if( !$user = User::find( 'one', array('select' => 'id, name, avatar, email, password, active, access_token', 'where' => array('email = ? AND password != ? AND facebook_id = ? AND google_id = ?', $posts['email'], '', '', '') ) ) )
   ...
   * @apiSuccess {String} token        Access Token
   * @apiSuccess {Object} user         登入的使用者
   * @apiSuccess {Number} user.id      使用者的 ID
   * @apiSuccess {String} user.name    使用者的名稱
   * @apiSuccess {String} user.avatar  使用者的頭像
   * @apiSuccess {String} type         使用者登入類型[normal, facebook, google]
   * @apiSuccessExample {json} 成功:
   *     HTTP/1.1 200 OK
   *     {
